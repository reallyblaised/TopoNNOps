...
class EnhancedDataPreprocessor:
    ...
    def fit(self, df: pd.DataFrame, balance_first: bool = True, min_count: Optional[int] = None) -> 'EnhancedDataPreprocessor':
        """
        ...
        self._plot_feature_distributions(df[feature_cols], "raw", "Raw Data Distributions")
        ...
        if balance_first:
            ...
            self.logger.info(f"Original data size: {len(df)} -> Balanced data size: {len(fit_df)}")
            if 'channel' in df.columns:
                channel_before = df['channel'].value_counts()
                channel_after = fit_df['channel'].value_counts()
                for channel in sorted(set(channel_before.index) | set(channel_after.index)):
            ...
            self._plot_feature_distributions(fit_df[feature_cols], "balanced", "Balanced Data Distributions")
        else:
            fit_df = df
        ...
        for column, transform_type in self.transformations.items():
            ...
            self.logger.info(f"Processing feature '{column}' with transform '{transform_type}'")
            ...
            if transform_type == "gev":
                ...
                self.logger.info(f"Applying log transform to '{column}' with epsilon={epsilon}")
            ...
            self.logger.info(f"After transform stats for '{column}': {transformed_stats}")
            
            # Calculate clipping bounds
            lower_bound = np.nanquantile(values, self.clip_quantiles[0])
            upper_bound = np.nanquantile(values, self.clip_quantiles[1])
            self.logger.info(f"Clipping bounds for '{column}': lower={lower_bound}, upper={upper_bound}")
            ...
            if self.normalize:
                ...
                self.logger.info(f"Normalization range for '{column}': min={min_val}, max={max_val}")
        ...
        self._plot_feature_distributions(transformed_df[feature_cols], "transformed", 
        ...
        self._plot_feature_distributions(clipped_df[feature_cols], "clipped", 
                                      "After Clipping (Quantiles)")
    ...
    def transform(self, df: pd.DataFrame, preserve_originals: bool = True) -> pd.DataFrame:
        ...
        if 'raw' not in self.snapshots:
            self.snapshots['raw'] = df[feature_cols].copy()
            self._plot_feature_distributions(df[feature_cols], "raw", "Raw Input Data Distributions")
        ...
        for column, transform_type in self.transformations.items():
            ...
            if transform_type == "gev":
                ...
                self.logger.info(f"Applied log transform to '{column}' with epsilon={epsilon}")
            elif transform_type == "none":
            ...
            if lower is not None and upper is not None:
                ...
                perc_clipped = ((lower_clipped + upper_clipped) / len(before_clip)) * 100
                
                self.logger.info(f"Clipped '{column}' to [{lower}, {upper}]")
                self.logger.info(f"  - Values clipped below: {lower_clipped} ({lower_clipped/len(before_clip)*100:.2f}%)")
                self.logger.info(f"  - Values clipped above: {upper_clipped} ({upper_clipped/len(before_clip)*100:.2f}%)")
                self.logger.info(f"  - Total clipped: {lower_clipped + upper_clipped} ({perc_clipped:.2f}%)")
            ...
            if self.normalize:
                ...
                if min_val is not None and max_val is not None and max_val > min_val:
                    ...
                    self.logger.info(f"Normalized '{column}' from [{min_val}, {max_val}] to [0, 1]")
                    
                    # Check transformed range
                    actual_min = result_df[column].min()
                    actual_max = result_df[column].max()
                    if actual_min < 0 or actual_max > 1:
                        self.logger.warning(f"  ⚠️ Normalized range outside [0,1]: [{actual_min}, {actual_max}]")
        ...
        if processed_features > 0:
            ...
            self._plot_feature_distributions(result_df[feature_cols], "final", "Final Transformed Data")
            self._plot_transformation_stages()
            self._plot_before_after_comparisons()
            self._generate_report()
            
        self.logger.info(f"Transform completed: {processed_features} features processed")
        return result_df
    
    def fit_transform(self, df: pd.DataFrame, balance: bool = True, 
                     min_count: Optional[int] = None, 
                     preserve_originals: bool = True) -> pd.DataFrame:
        """
        ...
        if balance:
            ...
            # Log balancing results
            self.logger.info(f"Original size: {len(df)} -> Balanced size: {len(balanced_df)}")
            if 'channel' in df.columns:
                ...
                for channel, row in channel_counts.iterrows():
                    self.logger.info(f"Channel '{channel}': {row['Original']} -> {row['Balanced']}")
    ...
    def balance_signal(self, df: pd.DataFrame, min_count: Optional[int] = None,
        ...
        if 'channel' not in df.columns:
            self.logger.warning("No 'channel' column found in dataframe, returning original data")
            return df
        ...
        self.logger.info(f"Identified {len(signal_channels)} signal channels: {signal_channels}")
        ...
        if self.background_channel in df['channel'].unique():
            ...
            self.logger.info(f"Keeping all {len(background_df)} background ({self.background_channel}) entries")
        ...
        for channel in signal_channels:
            ...
            if len(channel_df) >= min_count:
                ...
                self.logger.info(f"Channel '{channel}': sampled {min_count} from {len(channel_df)}")
            else:
                # If not enough entries, take all available
                balanced_df = pd.concat([balanced_df, channel_df])
                self.logger.warning(f"Channel '{channel}' has only {len(channel_df)} entries, less than target {min_count}")
    ...
    @classmethod
    def load(cls, filename: str, log_dir: str = "./preprocessing_logs") -> 'EnhancedDataPreprocessor':
        ...
        try:
            ...
            preprocessor.logger.info(f"Loaded {len(preprocessor.feature_stats)} feature statistics")
    ...
    def _plot_feature_distributions(self, df: pd.DataFrame, stage: str, title: str) -> None:
        ...
        self.logger.info(f"Saved feature distributions for '{stage}' stage to {plot_path}")
    
    def _plot_channel_distribution(self, original_df: pd.DataFrame, balanced_df: pd.DataFrame) -> None:
        ...
        for i, p in enumerate(ax.patches):
            ...
            x, y = p.get_xy() 
            ax.annotate(f'{height:g}', (x + width/2, y + height + 0.1), 
    ...
    def _plot_transformation_stages(self) -> None:
        ...
        stages = [stage for stage in ['raw', 'balanced', 'transformed', 'clipped', 'final'] 
        ...
        for feature in feature_cols:
            ...
            self.logger.info(f"Saved transformation statistics for '{feature}' to {plot_path}")
    ...
    def _plot_before_after_comparisons(self) -> None:
        ...
        for feature in feature_cols:
            if feature not in self.snapshots[before_stage].columns or feature not in self.snapshots[after_stage].columns:
                continue
            ...
            self.logger.info(f"Saved before-after comparison for '{feature}' to {plot_path}")
    ...
    def _generate_report(self) -> None:
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"- Log transform variables: {', '.join(self.log_vars) or 'None'}\n")
            f.write(f"- Unchanged variables: {', '.join(self.unchanged_vars) or 'None'}\n")
            ...
            f.write("| Feature | Transform | Epsilon | Lower Bound | Upper Bound | Min Value | Max Value |\n")
            f.write("|---------|-----------|---------|-------------|-------------|-----------|----------|\n")
            ...
            for feature in sorted(self.transformations.keys()):
                ...
                f.write(f"| {feature} | {transform} | {epsilon} | {lower} | {upper} | {min_val} | {max_val} |\n")
            ...
            for stage in stages:
                if os.path.exists(os.path.join(self.log_dir, f"distribution_{stage}.png")):
                    f.write(f"- [{stage.capitalize()} Data Distributions](distribution_{stage}.png)\n")
            
            # Feature transformation links
            f.write("\n### Transformation Statistics\n\n")
            for feature in sorted(self.transformations.keys()):
                if os.path.exists(os.path.join(self.log_dir, f"stats_changes_{feature}.png")):
                    f.write(f"- [{feature} Statistics](stats_changes_{feature}.png)\n")
            
            # Before-after comparison links
            f.write("\n### Before-After Comparisons\n\n")
            for feature in sorted(self.transformations.keys()):
                if os.path.exists(os.path.join(self.log_dir, f"before_after_{feature}.png")):
