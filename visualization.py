...
class ModelPerformance:
    ...
    def create_performance_dashboard(
        ...
        preprocessor: Optional[
            object
        ] = None,  # fetch the preprocessor to invert the [0,1]-normalisation
    ) -> None:
        """
        ...
        if channels is not None:
            if len(channels) != len(X_test):
                print(
                    f"WARNING: Channel length mismatch. Channels: {len(channels)}, X: {len(X_test)}"
                )
        ...
        dashboard_components = [
            alt.hconcat(learning_chart, metrics_chart).resolve_scale(
                color="independent"
            ),
            alt.hconcat(feature_importance, confusion_matrix).resolve_scale(
                color="independent"
            ),
        ...
        if channels is not None:
            try:
                ...
                if "TwoBody_PT" in self.feature_names:
                    ...
                    dynamic_efficiency_chart = (
                        self._create_dynamic_threshold_efficiency_vs_pt(
                            X_test, y_test, channels, y_test_pred, pt_values, pt_index
                        )
                    )
                ...
                df = pd.DataFrame(
                    [{"message": f"Error creating efficiency chart: {str(e)}"}]
                )
                placeholder = (
                    alt.Chart(df)
                    .mark_text()
    ...
    def _create_metric_evolution(self, history: dict) -> alt.Chart:
        ...
        chart = (
            ...
            .properties(width=400, height=300, title="Validation Metrics Evolution")
            .interactive()
        )
    ...
    def _create_feature_importance(self, X: np.ndarray, y: np.ndarray) -> alt.Chart:
        ...
        for i, feature_name in enumerate(self.feature_names):
            ...
            importance_data.append({"Feature": feature_name, "Importance": corr})
        ...
        chart = (
            ...
            .properties(width=400, height=300, title="Feature Importance (Correlation)")
            .interactive()
        )
    ...
    def _create_confusion_matrix(
        ...
        cm_data = [
            {
                ...
                "Actual": "Negative",
                "Count": int(tn),
                "Normalized": float(tn) / float(total),
            },
            {
                "Predicted": "Positive",
                "Actual": "Negative",
                "Count": int(fp),
                "Normalized": float(fp) / float(total),
            },
            {
                "Predicted": "Negative",
                "Actual": "Positive",
                "Count": int(fn),
                "Normalized": float(fn) / float(total),
            },
            {
                "Predicted": "Positive",
                "Actual": "Positive",
                "Count": int(tp),
                "Normalized": float(tp) / float(total),
            },
        ]
        ...
        base = alt.Chart(df).encode(
            x=alt.X(
                "Predicted:N", title="Predicted Class", sort=["Negative", "Positive"]
            ),
        ...
        class_sizes_text = (
            ...
            .mark_text(align="center", baseline="top", fontSize=12)
            .encode(x=alt.value(200), y=alt.value(310), text="text:N")
        )
        ...
        sanity_text = (
            ...
            .mark_text(
                ...
                color=(
                    "green"
                    if all(metrics_match.values()) and matrix_total_check
                    else "red"
                ),
            )
            .encode(
        ...
        matrix = (heatmap + text).properties(
            width=400, height=250, title="Confusion Matrix (Globally Normalized)"
        )
    ...
    def _create_roc_curve(self, y_true: np.ndarray, y_pred: np.ndarray) -> alt.Chart:
        ...
        try:
            ...
            for i in range(len(fpr)):
                thresh_val = (
                    thresholds[i - 1] if i > 0 and i < len(thresholds) + 1 else 0
                )
                data.append(
                    {
            ...
            text = (
                alt.Chart(
                    pd.DataFrame(
                        {"x": [0.6], "y": [0.2], "text": [f"AUC = {roc_auc:.3f}"]}
                    )
                )
            ...
            return (
                ...
                .encode(text="message:N")
                .properties(width=400, height=300, title="ROC Curve (Failed)")
            )
    ...
    def _create_pr_curve(self, y_true: np.ndarray, y_pred: np.ndarray) -> alt.Chart:
        ...
        try:
            ...
            for i in range(len(precision)):
                thresh_val = (
                    thresholds[i - 1] if i > 0 and i < len(thresholds) + 1 else 1.0
                )
                data.append(
                    {
            ...
            line = (
                ...
                .encode(
                    ...
                    y=alt.Y(
                        "Precision:Q", title="Precision", scale=alt.Scale(zero=False)
                    ),
            ...
            text = (
                alt.Chart(
                    pd.DataFrame(
                        {"x": [0.6], "y": [0.9], "text": [f"AP = {avg_precision:.3f}"]}
                    )
                )
                .mark_text(align="left", baseline="middle")
                .encode(x="x:Q", y="y:Q", text="text:N")
            ...
            # Combine all elements
            chart = (
                (line + baseline_line + text)
                .properties(width=400, height=300, title="Precision-Recall Curve")
                .interactive()
            )
            ...
            return (
                ...
                .mark_text(align="center", baseline="middle")
                .encode(text="message:N")
                .properties(width=400, height=300, title="PR Curve (Failed)")
            )

    def _create_response_distribution(
        self, y_true: np.ndarray, y_pred: np.ndarray
    ) -> alt.Chart:
        ...
        signal_df = pd.DataFrame(
            {"Response": y_pred.ravel()[y_true.ravel() == 1], "Class": "Signal"}
        )

        background_df = pd.DataFrame(
            {"Response": y_pred.ravel()[y_true.ravel() == 0], "Class": "Background"}
        )
                ...
                color=alt.Color(
                    ...
                    scale=alt.Scale(
                        ...
                        range=[
                            "#1f77b4",
                            "#d62728",
                        ],  # Blue for background, red for signal
                    ),
                ),
    ...
    def _create_channel_efficiency_vs_pt(
            ...
            return (
                alt.Chart(
                    pd.DataFrame(
                        {
                            "message": [
                                f"Channel data shape mismatch: X shape={X.shape}, channels shape={channels.shape if channels is not None else None}"
                            ]
                        }
                    )
                )
            ...
            if pt_index is None:
                if "TwoBody_PT" not in self.feature_names:
                    ...
                    return (
                        alt.Chart(
                            pd.DataFrame(
                                {"message": ["TwoBody_PT not found in features"]}
                            )
                        )
                        .mark_text()
                        .encode(text="message:N")
        ...
        if hasattr(self, "preprocessor") and self.preprocessor is not None:
            ...
            if (
                hasattr(self.preprocessor, "normalize")
                and self.preprocessor.normalize
                and hasattr(self.preprocessor, "feature_stats")
            ):
            ...
            return (
                alt.Chart(
                    pd.DataFrame(
                        {
                            "message": [
                                f"Shape mismatch: minbias_mask={minbias_mask.shape}, predictions={flattened_preds.shape}"
                            ]
                        }
                    )
                )
        ...
        for cut in cut_values:
            ...
            rejection_rate = (
                minbias_rejected / minbias_total if minbias_total > 0 else 0
            )
                    ...
                    efficiency_data.append(
                        {
                            ...
                            "Cut": str(
                                cut
                            ),  # Convert to string for proper legend grouping
            ...
            return (
                alt.Chart(
                    pd.DataFrame(
                        {
                            "message": [
                                "No efficiency data available for selected channels"
                            ]
                        }
                    )
                )
        ...
        for i in range(len(hist)):
            ...
            pt_dist_data.append({"PT_Bin": bin_center, "Normalized_Height": hist[i]})
        ...
        for cut in cut_values:
            ...
            eff_base = alt.Chart(cut_df).encode(x=alt.X("PT_Bin:Q"))
            ...
            lines = eff_base.mark_line().encode(
                y=alt.Y(
                    "Efficiency:Q", title="Efficiency", scale=alt.Scale(domain=[0, 1])
                ),
            ...
            for channel in selected_channels:
                if (
                    channel in overall_efficiencies
                    and cut in overall_efficiencies[channel]
                ):
            ...
            text_marks = (
                alt.Chart(pd.DataFrame(text_data))
                .mark_text(
                    align="center",
                    baseline="middle",
            ...
            combined_chart = alt.layer(
                pt_area, bands, lines, points, text_marks
            ).properties(
                width=400, height=300, title=f"Channel Efficiency vs PT (Cut = {cut})"
            )
        ...
        if cut_charts:
            ...
            title_chart = (
                alt.Chart(
                    pd.DataFrame(
                        [
                            {
                                "text": "Signal Channel Efficiency vs PT with Uncertainty Bands"
                            }
                        ]
                    )
                )
                .mark_text(fontSize=16, font="Arial", fontWeight="bold")
                .encode(text="text:N")
            )
            ...
            if len(cut_charts) >= 4:
                final_chart = alt.vconcat(
                    alt.hconcat(cut_charts[0], cut_charts[1]).resolve_scale(
                        color="shared"
                    ),
                    alt.hconcat(cut_charts[2], cut_charts[3]).resolve_scale(
                        color="shared"
                    ),
                ...
                final_chart = alt.hconcat(cut_charts[0], cut_charts[1]).resolve_scale(
                    color="shared"
                )
            else:
            ...
            return (
                alt.Chart(pd.DataFrame({"message": ["No efficiency data available"]}))
                .mark_text()
                .encode(text="message:N")
                .properties(width=800, height=200)
            )
    ...
    def _log_dashboard(self, dashboard, epoch: int) -> None:
        ...
        try:
            ...
            # Log to MLflow
            mlflow.log_artifact(
                temp_path, f"visualization/performance_dashboard_epoch_{epoch}"
            )
    ...
    def _create_feature_correlation_matrices(
        ...
        signal_corr_df = pd.DataFrame(
            signal_corr, columns=self.feature_names, index=self.feature_names
        )
        background_corr_df = pd.DataFrame(
            background_corr, columns=self.feature_names, index=self.feature_names
        )

        # Reshape for visualization (long format)
        signal_corr_long = signal_corr_df.reset_index().melt(
            id_vars="index", value_name="correlation", var_name="feature"
        )
        ...
        text = base.mark_text(baseline="middle").encode(
            ...
            color=alt.condition(
                abs(alt.datum.correlation) > 0.7, alt.value("white"), alt.value("black")
            ),
        )

        # Combine heatmap and text
        matrix = (heatmap + text).properties(width=400, height=400)
    ...
    def _find_threshold_for_rejection_rate(
        self, y_pred: np.ndarray, channels: np.ndarray, target_rejection_rate: float
    ) -> float:
        """
    ...
    def _create_dynamic_threshold_efficiency_vs_pt(
            ...
            return (
                alt.Chart(
                    pd.DataFrame(
                        {
                            "message": [
                                f"Channel data shape mismatch for dynamic thresholds"
                            ]
                        }
                    )
                )
        ...
        for channel in selected_channels:
            ...
            for rate, threshold in dynamic_thresholds.items():
                ...
                for i in range(len(pt_bins) - 1):
                    ...
                    # Calculate binomial error
                    error = (
                        np.sqrt((efficiency * (1 - efficiency)) / total_events)
                        if total_events > 0
                        else 0
                    )
        ...
        if len(df) == 0:
            return (
                alt.Chart(
                    pd.DataFrame(
                        {"message": ["No dynamic threshold efficiency data available"]}
                    )
                )
                .mark_text()
                .encode(text="message:N")
        ...
        for rate in target_rejection_rates:
            ...
            # Create base for efficiency plots
            eff_base = alt.Chart(rate_df).encode(x=alt.X("PT_Bin:Q"))
            ...
            for channel in selected_channels:
                if (
                    channel in overall_efficiencies
                    and rate_key in overall_efficiencies[channel]
                ):
            ...
            text_marks = (
                alt.Chart(pd.DataFrame(text_data))
                .mark_text(align="center", baseline="middle", fontSize=8)
                .encode(
                    x="x:Q",
                    y="y:Q",
                    text="text:N",
        ...
        if rate_charts:
            ...
            title_chart = (
                alt.Chart(
                    pd.DataFrame(
                        [
                            {
                                "text": "Signal Channel Efficiency vs PT at Fixed Minbias Rejection Rates"
                            }
                        ]
                    )
                )
            ...
            if len(rate_charts) == 2:
                final_chart = alt.hconcat(rate_charts[0], rate_charts[1]).resolve_scale(
                    color="shared"
                )
            else:
